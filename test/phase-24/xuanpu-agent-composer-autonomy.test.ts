...
describe('xuanpu-agent composer autonomy directive', () => {
  it('infers explicit long task-run directives from composer text', () => {
    ...
    expect(
      inferXuanpuAgentTaskRunAutonomyDirective('按 long autonomy 执行。没做完不要停', 'xuanpu-agent')
    ).toBe('long')

    expect(inferXuanpuAgentTaskRunAutonomyDirective('/overnight 继续跑完整测试', 'xuanpu-agent')).toBe(
      'overnight'
    )
  })
