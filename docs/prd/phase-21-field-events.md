...
2. 提供 `emitFieldEvent()` 工具：**直接 enqueue 到 sink**（主路径）+ 在现有 `EventBus` 上 best-effort 广播 `'field:event'`（副路径，仅供未来订阅者使用）
3. 建立异步批量落盘的 `FieldEventSink`（buffer + 1s/100 条 flush + 退出前强制 flush + 失败重试 + 毒事件隔离 + 观测计数器）
4. 在 3 个关键切点接入 P0 事件集：`worktree.switch`、`terminal.command`、`session.message`
...
- ❌ `terminal.output`（窗口聚合 + command 关联，复杂度偏高）→ **降级到 Phase 21.5 / Phase 22**
- ❌ Agent system prompt 注入 → Phase 23
- ❌ Worktree 摘要 / 项目记忆文件 → Phase 22
- ❌ 前端 Field Timeline UI → Phase 22 或 24
- ❌ `file.open` / `file.selection` / `file.edit` / `cursor.position`（renderer 侧采集）→ Phase 22
- ❌ `git.status_change` / `session.approval` / `connection.*` → P1/P2
