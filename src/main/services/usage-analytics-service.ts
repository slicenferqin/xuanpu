...
export class UsageAnalyticsService {
  ...
  async fetchDashboard(filters: UsageAnalyticsFilters): Promise<UsageAnalyticsDashboardResult> {
    try {
      ...
      const sessions = this.db.getUsageAnalyticsSessions(
        ['claude-code', 'codex', 'xuanpu-agent'],
        sessionStatus
      )
  ...
  async fetchScopeSummary(
    ...
    try {
      ...
      const sessionContributions = new Map<
        string,
        {
          totalCost: number
          totalTokens: number
      ...
      for (const sessionId of sessionIds) {
        ...
        if (syncSnapshot.partial && syncSnapshot.reason) {
          partialSessions.push({
            ...
            session_name: session.name ?? 'Untitled',
            engine:
              session.agent_sdk as import('@shared/types/usage-analytics').UsageAnalyticsEngine,
  ...
  private async syncCodexSession(session: SupportedSession): Promise<'synced' | 'partial'> {
    ...
    const sourceMtimeMs =
      backfillResult?.sourceMtimeMs ?? (jsonlPath ? new Date(session.updated_at).getTime() : null)
    ...
    try {
      ...
      for (const line of lines) {
        try {
          ...
          const modelID = sessionModelId ?? resolveCodexModelSlug(asString(info.model) ?? undefined)
          ...
          const totalDelta =
            lastTokensDelta.input + lastTokensDelta.output + lastTokensDelta.cacheRead
          ...
          if (typeof rawTimestamp === 'string' && rawTimestamp.length > 0) {
            ...
            occurredAt = Number.isFinite(parsed.getTime())
              ? parsed.toISOString()
      ...
      if (lastTokenCount && result.insertedEventCount > 0) {
        ...
        if (lastUsage && totalUsage) {
          ...
          const modelID =
            sessionModelId ?? resolveCodexModelSlug(asString(lastTokenCount.model) ?? undefined)
          const pricingModelKey = resolvePricingModelKey(modelID, 'codex')
